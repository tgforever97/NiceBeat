...
import com.neo.sk.NiceBeat.core.RoomActor.{LeftRoomByKilled, WebSocketMsg}
...
object RoomManager {
    ...
    Behaviors.setup[Command]{
      ctx =>
        ...
        Behaviors.withTimers[Command]{implicit timer =>
          ...
          val roomInUse = mutable.HashMap((1l,List.empty[(String,String,ActorRef[UserActor.Command])]))
  ...
  def idle(roomIdGenerator:AtomicLong,
           roomInUse:mutable.HashMap[Long,List[(String,String,ActorRef[UserActor.Command])]])
      ...
      msg match {
        case JoinRoom(uid,tankIdOpt,name,startTime,userActor) =>
          roomInUse.find(p => p._2.length < personLimit).toList.sortBy(_._1).headOption match{
            case Some(t) =>
              roomInUse.put(t._1,(uid,name,userActor) :: t._2)
            ...
            case None =>
              ...
              roomInUse.put(roomId,List((uid,name,userActor)))
        ...
        case LeftRoom(uid,uName,boardId) =>
          roomInUse.find(_._2.exists(_._1 == uid)) match{
            case Some(t) =>
              ...
              getRoomActor(ctx,t._1) ! RoomActor.LeftRoom(uid,uName,boardId,roomInUse(t._1).map( u => (u._1,u._2)),t._1)
